...
@Tag("unit")
class FirServiceEvidenceUploadTest {

    private FirReportRepository firReportRepository;
    private FirService firService;

    @BeforeEach
    void setUp() {
        firReportRepository = mock(FirReportRepository.class);
    ...
    @Test
    void shouldAcceptWordDocumentEvidence() {
        ...
        assertDoesNotThrow(() -> firService.uploadEvidence(1L, file));
    }

    @Test
    void shouldRejectUnsupportedEvidenceType() {
        MockMultipartFile file = new MockMultipartFile(
                "file",
                "script.exe",
        ...
        assertThrows(IllegalArgumentException.class, () -> firService.uploadEvidence(1L, file));
    }
}
